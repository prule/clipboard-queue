# list-storage-location Specification

## Purpose
Where the app keeps its persisted lists file, and how it behaves when that file
or its directory is missing or unwritable. Covers the location only — what goes
in the file on a first launch is `starter-content`, and how the queue walks the
loaded list is `queue-advance`.
## Requirements
